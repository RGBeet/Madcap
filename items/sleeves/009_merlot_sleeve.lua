return {
    ...
    data = {
		...
		loc_vars = function(self)
			local key
			local config 	= MadLib.deep_copy(Madcap.DeckConfigs.merlot.base)
			local nf		= 13
            if self.get_current_deck_key() == "b_rgmc_merlot" then
                key = self.key .. "_dd"
			...
			elseif self.get_current_deck_key() == "b_abandoned" then -- 13*6 -> 10*6
				nf = 10
			elseif self.get_current_deck_key() == "b_rgmc_sangria" then -- Blooms, Daggers, Goblets, Towers
                key = self.key .. "_pl"
				config.special_deck = 'sangria'
			else
                key = self.key
			end
			local vars 		= {
		...
		apply = function(self) -- Start of the run
			...
			if self.config.special_deck then -- change every other card
				if 
					...
					or self.config.special_deck == 'sangria'
				then
					MadLib.event({
						func = function()
