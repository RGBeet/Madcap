...
function MadLib.get_card_setting_ability()
    ...
    return ret and G.STATE ~= G.STATES.RGMC_LUXURY_SHOPPE
end
    ...
    for _, v in ipairs(rates) do
        ...
        if polled_rate > check_rate and polled_rate <= check_rate + v.val then
            ...
            if v.type == 'Joker' then
                --print('JOKER TIME!')
                local _rarity = Madcap.Funcs.get_weighted_choice(Madcap.Lists.LuxuryRates)
                card = create_card(v.type, area, nil, _rarity, nil, nil, nil, 'sho')
                MadLib.event({
                    func = (function()
